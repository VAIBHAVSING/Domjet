    ...
    match method {
        ...
        "createTarget" => {
            ...
            if url_is_file_scheme(url) && !context.allow_file_access {
                return Err(
                    "Target.createTarget to file:// is disabled. Enable allow_file_access on the server context to allow it.".to_string()
                );
            }
        ...
        // No multi-target lifecycle to manage: Domjet runs one page per session.
